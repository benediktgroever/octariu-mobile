...
const Set = (props: SetProps) => {
    ...
    const [repCount, onChangeRepCount] = useState(props.set.repCount);
    ...
    const submitEditingRepCount = () => {
        updateSet({ setId: props.set.setId, repCount: repCount });
        Keyboard.dismiss();
    }
    ...
    return (
        <View style={finished ? [styles.set, styles.finishedSet] : styles.set}>
            ...
            <TextInput
                editable
                onChangeText={text => onChangeWeight(Number(text.replace(/[-+]?[0-9]*\.?[0-9]+/g, '')))}
            ...
            <TextInput
                editable
                onChangeText={text => onChangeRepCount(Number(text.replace(/[^0-9]/g, '')))}
                onSubmitEditing={submitEditingRepCount}
